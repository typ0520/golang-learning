...
func main() {
	...
	table["3[a]2[bc]"] = "aaabcbc"
	table["3[a2[c]]"] = "accaccacc"
	table["2[abc]3[cd]ef"] = "abcabccdcdcdef"
	table["abc3[cd]xyz"] = "abccdcdcdxyz"

	for k, v := range table {
		got := decodeString(k)
		if v != got {
			log.Fatal(fmt.Sprintf("error, input: %s, output: %s, expect: %s", k, got, v))
		}
	}
}
